...
#' calculates a weighted quantile, its variance, a 95\% confidence 
#' interval and three coefficients of variation
#' 
#' SPERIMENTALE (AGGIUNTA DELL'ARGOMENTO \code{probs} ALLA FUNZIONE 
#' \code{statMed} DELL'UST), PER INDICARE IL LIVELLO DEL QUANTILE PER IL QUALE 
#' SI DESIDERA STIMARE UN INTERVALLO DI CONFIDENZA.
#' 
#' Le descrizioni degli argomenti qui sotto sono state riprese da pag. 31 
#' (un po' in francese, un po' in italiano)
#' del documento metodologico dell'Ust (ESS, Programmes R pour l'intervalle 
#' de confiance de la MEDIANE)
#' 
#' @param x un vettore numerico (variable di studio di cui si stima la mediana) (esempio: salari (MBLS o MNLIU))
#' @param probs livello del quantile desiderato (numero compreso tra 0 e 1)
#' @param strata identificatore dello strato
#' @param psu identificatore dell'impresa (primary selection unit)
#' @param nh numero di imprese sondate nello strato h (dimensioni di campionamento nette degli strati - nel campione originale, indipendentemente da qualsiasi definizione di dominio)
#' @param th taux de sondage effectif dans la strate (nh / Nh)
#' @param Nh tailles totales des strates
#' @param mhi numero di salari sondati nell'impresa i (dimensioni di campionamento delle PSU - nel campione originale, indipendentemente da qualsiasi definizione di dominio) 
#' @param thi taux de sondage de l'entreprise i (tasso di campionamento delle PSU - mhi / Mhi)
#' @param Mhi Dimensioni totali delle PSU
#' @param weights pesi delle SSU (tenendo conto dei pesi di campionamento, ma non necessariamente uguali)
#' @param crit criterio utilizzato per l'imputazione della varianza per gli strati che contengono una sola PSU
#' @export
statQuantile <- function(x, probs = 0.5, strata = NULL, psu = NULL, nh = NULL,
                         th = NULL, Nh = NULL, mhi = NULL, thi = NULL, 
...
##################################################################### 
  ...
  CVperc <- 100 * sep / probs  
  # QUA NON SO SE SOSTITURIRE 0.5 con probs 
